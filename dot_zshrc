...
export PATH=$HOME/redis/src:$HOME/bin:/usr/local/bin:/usr/local/sbin:/usr/local/mysql/bin/:/opt/local/bin:$PATH
...
alias pp='pwd | pbcopy'
alias ack='ack-grep'
alias fx6='~/opt/firefoxes/firefox/firefox -no-remote -P firefox-6.0'
alias jim='~/Jim/jim'
alias refreshrbtags="ctags -f tags --recurse=yes . && find . -name '*.rb' -o -name '*.java' -o -name '*.css' -o -name '*.js' -o -name '*.haml' -o -name '*.erb' > cscope.files && cscope -b -q"
alias refreshctags="ctags -f tags --recurse=yes . && find . -name '*.c' -o -name '*.h' -o -name '*.cpp' -o -name '*.cc' -o -name '*.rb' > cscope.files && cscope -b -q"
